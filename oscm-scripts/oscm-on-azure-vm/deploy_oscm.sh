...
install_oscm () {
  ...
  sudo docker run --name deployer1 --rm -v /docker:/target -e HOST_FQDN=$publicIP -e SAMPLE_DATA=true servicecatalog/oscm-deployer
  ...
  sudo docker run --name deployer2 --rm -v /docker:/target -v /var/run/docker.sock:/var/run/docker.sock -e INITDB=true -e STARTUP=true -e SAMPLE_DATA=true -e PROXY=true docker.io/servicecatalog/oscm-deployer
