...
if [ ${PROXY_ENABLED} -eq 1 ]; then
	docker build -t oscm-db:${GIT_SOURCE} --build-arg HTTP_PROXY="http://${HTTP_PROXY_HOST}:${HTTP_PROXY_PORT}" --build-arg HTTPS_PROXY="http://${HTTPS_PROXY_HOST}:${HTTPS_PROXY_PORT}" oscm-dockerbuild/oscm-db
else
