...
function genPropertyFilesJMS {
    ...
    /usr/bin/envsubst < /opt/templates/config.properties.core.template > /opt/glassfish4/glassfish/domains/core-domain/imq/instances/imqbroker/props/config.properties
...
if [ $TARGET == "JMS" ]; then
	...
	if [ $SOURCE == "INIT" ]; then
        ...
        /opt/glassfish4/mq/bin/imqdbmgr recreate tbl -varhome /opt/glassfish4/glassfish/domains/core-domain/imq -javahome /usr/lib64/jvm/java
	fi
